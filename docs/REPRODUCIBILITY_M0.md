...
> **СТАТУС (M3, C061): ИСТОРИЧЕСКИЙ ДОКУМЕНТ.** Он описывает прогон схемы
> артефактов `v1`, записанный в `docs/baselines/M0/**`. Эти артефакты не
> изменялись и не будут: провалы прошлого не переписываются. Но с C059/C061
> runner пишет схему `v3` (declared assets + normative/informational split),
> поэтому воспроизвести ИМЕННО эти файлы можно только на M0-коммитах
> C002–C005 — что для `env.json` было верно уже с C009.
>
> **Действующая инструкция — `docs/REPRODUCIBILITY_M3.md`**, вместе с
> `docs/baselines/M3/**` и командой `baseline-runner compare-hashes`.
