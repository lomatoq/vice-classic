...
### Addendum M2 — `ValidatedScene` (REVIEW_M1 M1-N10)

Решение: РЕАЛИЗОВАН newtype `validate::ValidatedScene` (приватное поле,
единственный конструктор `new` = `validate_scene`). Основание: появился
второй call site, зависящий от предусловия «граф прошёл валидацию» —
renderer M2 (общая tessellation границ и partition-инварианты корректны
только на валидном графе), в дополнение к
`uncertified_interference_pairs`, чья позиционная смежность точна только
после chain-point distinctness. Обе точки теперь принимают
`&ValidatedScene`: предусловие выражено типом, а не доккомментарием.
`validate_scene`/`validate_graph` остаются публичными для негативных
тестов и прямых проверок; `canonical_scene_bytes` по-прежнему валидирует
внутренне (его вход — произвольная сцена по контракту M1; менять форму
M1-API без необходимости не стали).
