...
# ADDENDUM 5 — RED TEAM M4.5, дельта-5 (C222–C223)

> Addendum публикуется **дословно**. Подписанный текст и addendum 1–4 выше не изменены ни в одной строке.
...
$ git rev-parse HEAD
047d0dc2f9855fc7f14adb9d4a7126298a61651d

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic 047d0dc [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**Изоляция.** Клон `…/scratchpad/rt6-c93a5f`, `CARGO_TARGET_DIR = …/scratchpad/tgt-rt6-c93a5f`; перед созданием проверено `ls -d …/rt6-* …/tgt-rt6-*` — пусто. `rt5-4b81e7` не переиспользовался. `git worktree prune` не запускался. Клон и target удалены. Донорские исходники не открывались (D-3).

**База на `047d0dc`:**

```text
cargo test --release --workspace                       504 passed, 0 failed
hygiene 11 passed / doc_claims 6 passed
cargo fmt --all --check                                чисто
cargo clippy --workspace --all-targets -- -D warnings   0 диагностик
gt-corpus topology-check                               reproduced, EXIT=0
```

Гейт-файл дельтой-5 не тронут; `crates/vice-topology/` не тронут — детерминизм §11.2 бит-в-бит тот же, что измерен в addendum 2.

---

## §1. Состояние прежних атак

| # | что атаковалось | вердикт на `047d0dc` |
|---|---|---|
| A1, A2, A9, A10, A12, A13, A14, A15, A16, A17, A18, A20, A21 | — | **ОТБИТЫ** (исполнением, см. ниже) |
| **RT45-A22** | понижение яруса строки | **ОТБИТА** |
| **RT45-A24** | паддинг правдоподобным классом | **ОТБИТА** для паддинга ВНЕ `envelope_classes`; класс — RT45-A28 |
| **RT45-A19** | продукт как тип std | ПРОХОДИТ — **объявлен, цена честная** |
| **RT45-A25 / A26 / A27** | F-1 / F-3 / F-4 | ПРОХОДЯТ — **заморожены, цены проверены** |

Исполнение, компактно:

```text
A15  expected `&LegalProfile`, found `RasterProfile`; expected `&LegalCell`, found `&DegradationCell`
A9   type `GtSourceGroup` is more private than the item `Rt45F` / `rt45_alias` / `Rt45B::groups`
A14  type `RenderedFixture` is more private than the item `rt45_ren`
A20  the split policy holds out "tiny-skia", which resolves to no RasterProfile variant
A16  the_report_aggregates_agree_with_the_run_they_came_from ... FAILED
A10  clause 1 turns green at 8 arms while the gate file registers 20
A17  row "| T1 " position 5: the row says 56, but recall_all.hits is 100
A18  no_workflow_redirects_the_gate_file ... FAILED
A2   the_independent_chain_agrees_with_the_production_signature ... FAILED
A1   [NOT MET] GT-equivalent topology present in envelope
A12  [NOT MET] GT-equivalent topology present in envelope
```

### RT45-A22 — ОТБИТА

Дословная атака дельты-4 (понижение `T1` до `membership` + перефраз + подделка цифрами):

```text
the_delta_clause_rows_equal_their_declared_keys_position_by_position ... FAILED
```

Правило `declared[0] != "positional"` накрывает оба непроверяемых яруса. Это годная форма: свойство, а не перечень.

### RT45-A24 — ОТБИТА для паддинга вне `envelope_classes`

```diff
// crates/vice-bench/src/topology/ambiguity.rs, ПОСЛЕ вызова envelope_classes
+                if bf { full.truncate(1); }
+                while bf && full.len() < 2 { full.push((3, 1)); }
```
```text
every_published_class_came_from_the_envelope ... FAILED
  pair adv/ambiguous/bridge-or-gap publishes [(1, 0), (3, 1)] and its envelope
  produces [(1, 0), (2, 0)]
```

Константа правдоподобия удалена, дедуп на её месте, а свойство проверяется повторным вопросом к конверту. Мой паддинг `(3, 1)` теперь роняет тест — ровно как заявлено в `T28d`.

---

## §2. Новые атаки

### RT45-A28 — «конверт спрашивают заново» тем же вызовом; инъекция ВНУТРИ него невидима — **MAJOR**

**Что заявлено.** `T28d`: «`every_published_class_came_from_the_envelope` зовёт `envelope_classes` заново — **тем же вызовом и с тем же конфигом**, что и `ambiguity.rs` — и требует РАВЕНСТВА». Докблок теста: «The input has to be the ENVELOPE, so this asks the envelope again — the same call `ambiguity.rs` makes, with the same config».

**Почему это не независимая проверка.** «Тот же вызов» — не усиление, а описание дефекта. Истина и проверяемая величина считаются ОДНОЙ функцией: это F-0034 / RT45-A2 дословно, класс, ради которого в этом же проекте существует `independent.rs`. Любое преобразование внутри `envelope_classes` видно обеим сторонам одинаково.

**Процедура воспроизведения.** Инъекция на одну строку глубже, чем в RT45-A24:

```diff
// crates/vice-bench/src/topology/ambiguity.rs, ВНУТРИ envelope_classes
-    Ok(p.envelope.signature_classes().into_iter().collect())
+    let mut classes: Vec<(u32, u32)> = p.envelope.signature_classes().into_iter().collect();
+    if !classes.contains(&(3, 1)) {
+        classes.push((3, 1));
...
```text
$ cargo test --release -p vice-bench topology::tests
  test result: ok. 10 passed; 0 failed          ← including every_published_class_came_from_the_envelope
$ gt-corpus topology --scope full
  [MET] ambiguous fixtures retain alternatives
$ cargo test --release -p vice-bench --test hygiene   11 passed
$ cargo test --release --workspace                   504 passed / 0 failed
...
$ gt-corpus topology-check --report docs/gt/TOPOLOGY_M4_5.json
  topology report did NOT reproduce                  EXIT=1     ← ЕДИНСТВЕННОЕ, что заметило
```

Класс `(3, 1)`, которого конверт не производил, опубликован в списке; порог `gate_min_classes_per_retaining_pair` сравнивается с ним; страж, написанный ровно чтобы это поймать, зелёный. Заметил только замороженный артефакт — то есть изменившееся ЧИСЛО, а не заявленное свойство. Артефакт делал это и до дельты-5.

**Калибровка, обязательная к публикации.** Я пробовал сделать инъекцию невидимой и для артефакта (срабатывание только при коротком конверте) — не вышло: `ambiguity/hole` публикует один класс, поэтому условный паддинг срабатывает и там, и артефакт всё равно движется. Механизм T28d не ловит инъекцию ни в одной из проверенных форм; ловит артефакт. Это ограничивает практический ущерб и не спасает заявление.

**Общее правило класса.** «Спросить заново тем же вызовом» — проверка на детерминизм функции, а не на правильность значения. Независимость обязана распространяться на цепь: вход сайта должен считаться ДРУГИМ путём — например прямым `propose(...)` на том же наблюдении, без обёртки, которую проверяют, — ровно так, как `independent.rs` устроен для сигнатуры.

---

### RT45-A29 — «величина, а не начертание» реализовано детектором ASCII-цифр — **MAJOR**

**Что заявлено.** `T29d`: «правило… говорит о ВЕЛИЧИНЕ, а не о начертании: слова читает так же мало, как цифры». Комментарий в коде: «A row in an unchecked tier publishes no measured quantity in ANY notation».

**Чем это реализовано.** `numbers.is_empty()`, где `numbers = row_numbers(row_line)`, а `row_numbers` берёт токен через `trim_matches(|c| !c.is_ascii_digit())` и пропускает всё, что содержит букву. То есть детектор видит ровно `[0-9]`. Восемнадцать величин, выписанных словами, автор не обнаружил механизмом — он их УБРАЛ; код их не видит и сейчас.

**Процедура воспроизведения (а): непроверяемый ярус.** В строку `T10d` (объявлена `membership`) дописано в колонку Evidence:

```text
Достижимо снаружи sealed-audit групп: ⅩⅩⅠⅠ из ⅩⅩⅠⅠ; байт RGBA: ２５８０４８;
доля популяции ⅔; реколл упал вдвое.
```

— римские числа (U+2160-блок), полноширинные цифры (U+FF10-блок), вульгарная дробь, и сравнительная величина словом.
...
**Процедура воспроизведения (б), и она хуже: ПОЗИЦИОННЫЙ ярус.** Позиционная привязка требует `numbers.len() == keys.len()`, но `numbers` — те же ASCII-цифры. Дописано в конец Evidence строки `T1` — той, что помечена «клауза спеки 1» и несёт **PASS**:

```text
ИТОГ: recall ５６ из １３２; нетривиальных ８ из ３１; бюджет потерял ⅔ ответов.
```
...
Ложное измерение стоит в строке, выносящей вердикт клаузы спеки, ни к какому ключу не привязано, и все шесть механизмов молчат. Ярусная машинерия достроена; токенизатор, который её кормит, видит один алфавит из многих.

**Общее правило класса.** Пятая форма одной и той же атаки (A4 → A11 → A17 → A22 → A29), и каждый раз чинился ЯРУС, а не то, что ярус читает. Проверяемая замена не требует таблицы нотаций: строка гейт-таблицы обязана состоять ТОЛЬКО из привязанных токенов и прозы без величин — то есть проверять надо не «есть ли цифра», а «весь ли текст ячейки разобран», и всё неразобранное, что не является заведомой прозой, объявлять ошибкой. Это критерий, а не список алфавитов.

---

## §3. Честность пяти замороженных цен

Проверял каждую исполнением, а не чтением.

| # | заявленная цена | моя проверка | вердикт |
|---|---|---|---|
| **F-1** | две строки `run:`: правка + `git commit -am` | воспроизведено в addendum 4 дословно | **точна** |
| **F-2** | шаг «измеряет» может не собраться; по коду возврата неотличимо | `cargo run --locked` — 22 из 22 измеряющих шагов, `cargo build` отдельным шагом — **0**; сборка и измерение по-прежнему в одной команде | **точна** |
| **F-3** | один шим в `PATH` | не воспроизводится на моей платформе; см. §4 | **честно названа непроверенной** |
| **F-4** | новый `pub(crate)` продуктовый тип + одна `pub fn` | повторил дословно: 63 рендера, 7 sealed-audit семейств, **258 048 байт** из `gt/grammar.rs`, при `fmt` чистом, clippy 0, hygiene 11/11 и 505 зелёных | **точна** |
| **F-5** | `audit-status` и `report` берут гейт-файл откуда угодно, но вердикта по §28 не выносят | измерено ниже | **занижения нет; формулировка неточна** |

**F-5, измерение.** Гейт-файл вне репозитория:

```text
report       --gates ../outside.toml   EXIT=0      ← принимает
audit-status --gates ../outside.toml   EXIT=0      ← принимает
topology     (GATE_PATHS[0])                       ← сверяет с HEAD, отказывает на правленом
```

Затем — подделанный файл (`min_accepted_source_groups_zero_failures` 459 → 1, `confidence` 0.99 → 0.10), поданный в `report`:

```text
M3 gate table:
  [MET] no test leakage …
  [MET] sealed-audit burn policy active …
  [MET] source-group independence defined …
  [MET] supported universe is finite/versioned …
  [MET] correlation-aware likelihood protocol exists …
EXIT=0

изменившиеся поля scorecard: 1
  /hashes/gates_sha256: 08b30194… -> 01fe8475…
```

**Ровно одно поле** — хеш самого файла. Ни одна строка §28 M3 не сдвинулась, `required_groups` осталось 459 (оно ВЫВОДИТСЯ, как и написано в гейт-файле), вердикт не изменился. Так что риск оценён верно.

Неточна одна фраза: «ни один **не выносит вердикт по клаузе §28**». `report` печатает «M3 gate table» с пятью `[MET]` — это вердикты §28 M3. Правильная формулировка — «значения гейт-файла не решают ни одной из этих строк; файл входит в них только хешем». Разница между «не выносит вердикт» и «выносит вердикт, не зависящий от подделываемых значений» существенна, и второе — то, что я измерил и что верно.

**Отдельно про F-3 и цену F-1, как просил governor.** Автор пишет: «вся сверка F-1 опирается на то, что `git` в `PATH` есть настоящий `git`». Это верно и это моя территория, поэтому говорю прямо: `load_for_a_gate_decision` вызывает `std::process::Command::new("git")`, то есть разрешает имя через `PATH`, а `PATH` — то же самое, чем распоряжается шаг workflow. Если гипотеза F-3 верна, то F-1 стоит **не две строки, а одну**, и весь механизм дельты-4 — сверка с `HEAD` — держится на предположении, которое никто не проверил. Я свою атаку не воспроизвёл и не выдаю её за подтверждённую; но заявленная цена F-1 в этом случае завышена в пользу автора, а не занижена, и структурная зависимость названа им самим. Занижения нет ни в одной из пяти.

---

## §4. F-0048, применённый к механизмам дельты-5

| механизм | Q1: литерал-перечень? | Q2: следующая находка | Q3: судья | вердикт |
|---|---|---|---|---|
| `every_published_class_came_from_the_envelope` | нет | — | **та же функция, что под проверкой** | **не проходит по Q3** (RT45-A28) |
| сайт `min_classes_per_retaining_pair` (дедуп) | нет — константа удалена | — | сравнение двух чтений одного списка | **проходит** для той узкой роли, которую теперь заявляет |
| «непроверяемый ярус не публикует величину» | **да** — алфавит `is_ascii_digit` | добавить нотацию | детектор глифов | **не проходит по Q1/Q2** (RT45-A29) |

Два новых механизма, оба не проходят вопрос, который автор сам написал; третий (сужение сайта до дедупа с честным перенесением свойства в отдельный тест) — проходит и является правильным ходом.

Отдельно засчитываю то, чего в милестоуне раньше не было: **две прескрипции отменены измерением, а не мнением**, и обе отмены я проверяю как корректные. Вывод конвейера по аргументу действительно даёт 10 → 24 → 30 модулей — я вижу причину в коде (`fns_matching` ищет вызовы ПО ИМЕНИ, а имена вроде `of`, `identifiability` сталкиваются по воркспейсу), и дискриминатор, срабатывающий на `vice-evidence` и `vice-image`, дискриминатором не является. Отмена условия 31(б) обоснована ещё сильнее: у гейт-файла две роли, и требование отказывать на изменённом файле убило бы политику прожига §27.1, тест которой правит файл НАМЕРЕННО. Отклонить прескрипцию рецензента измерением — правильное поведение, и оно здесь исполнено дважды.

---

## §5. Суждение о разделении объёма

Governor просил судить это как заявку. Сужу: **разделение законно, и это не способ не закрывать закрываемое.**

Основания, по каждому пункту проверенные, а не принятые на слово:

1. **Все пять замороженных — действительно свойства билд-системы или требуют другого прибора.** F-1, F-2, F-3, F-5 упираются в `ci.yml` либо в разведение двух ролей гейт-файла типом; F-4 — в сканер вызовов, резолвящий имена в определения. Ни одно не закрывается правкой, которую M4.5 может проверить прогоном. Я искал форму, которая закрывалась бы здесь, и не нашёл.
2. **Цены не занижены** — все пять проверены выше, четыре точны, одна (F-5) сформулирована неточно в сторону, которая делает механизм СЛАБЕЕ на словах, чем он есть.
3. **Прескрипции отменены измерением, и измерения воспроизводимы.**
4. **Прецедент против.** Дважды в этом милестоуне завышенная оценка остатка становилась находкой; здесь оценки занижены не были ни разу.

Что мешает назвать дельту-5 закрытием: две вещи, замороженные НЕ были и заявлены закрытыми — `T28d` и `T29d`, — и обе не выполняются. Разделение объёма честное; исполнение той половины, которую автор оставил себе, — нет.

---

## §6. Атаки, которые НЕ удались

**FA. RT45-A24 вне `envelope_classes`.** Отбита, с точным сообщением, называющим пару, опубликованный список и то, что производит конверт.

**FB. RT45-A22.** Отбита; правило о ярусе — свойство, а не перечень.

**FC. Все тринадцать прежних атак** (§1) — отбиты, регрессий нет.

**FD. Прибор CI.** `gen-smoke --check`: `OK=0`, `MISSING=3`; `--locked` — 26 из 26.

**FE. Артефакт и детерминизм.** `topology-check` — reproduced, exit 0. `crates/vice-topology/` дельтой-5 не тронут.

**FF. §27.6 placeholders.** Нет.

**FG. Расхождение конверта в другую сторону.** Пробовал добиться, чтобы повторный вызов `envelope_classes` вернул НЕ ТО ЖЕ (недетерминизм на одном входе) — не удалось: функция чистая, `propose` детерминирован, повторный вызов совпадает. Расхождение достигается только инъекцией, и она видна артефакту (RT45-A28).

---

## §7. Что я не смог атаковать

1. **PATH-shim для `git`** (F-3) — по-прежнему не воспроизводится: `Command::new("git")` на Windows ищет `git.exe`, безрасширенный shim не подхватывается. Нужен ubuntu-раннер. Публикую как непроверенное второй раз подряд.
2. **CI прогоном.** Сеть не использовалась; run 67 не читал.
3. **Свип по бюджету конверта** — не сделан.
4. **Донорские исходники** — не открывались (D-3).
5. **Пять замороженных измерений `--ignored`** — выборочно.
6. **`achievable` внутри `SupportedModelUniverse`** — владелец M6/M7.

---

## §8. RED TEAM VERDICT (addendum 5)

**RED TEAM VERDICT (addendum 5): FAIL**

*(Дельта-5 сделала три вещи правильно, и я их называю первыми: RT45-A22 закрыта свойством, а не перечнем; константа правдоподобия удалена, и паддинг ВНЕ `envelope_classes` теперь роняет тест с точным сообщением; и впервые в милестоуне ДВЕ прескрипции — моя и рецензента — отменены измерением, которое я проверил и признаю корректным. Разделение объёма законно: все пять замороженных пунктов действительно упираются в билд-систему или в другой прибор, ни одна из пяти цен не занижена, а F-4 я перемерил дословно — 258 048 байт, 505 зелёных тестов.*

*Основание FAIL — два MAJOR, оба в той половине, которую автор оставил себе и объявил закрытой. RT45-A28: `T28d` заявляет, что класс обязан прийти ИЗ КОНВЕРТА, а конверт спрашивают ТЕМ ЖЕ вызовом, поэтому инъекция внутри `envelope_classes` невидима — десять тестов топологии зелены, клауза 2 MET, hygiene 11/11, 504 зелёных, и замечает только замороженный артефакт, то есть число, а не свойство; это F-0034 / RT45-A2 дословно, класс, ради которого в этом же проекте написан `independent.rs`. RT45-A29: `T29d` заявляет правило о ВЕЛИЧИНЕ, а не о начертании, а реализовано оно детектором ASCII-цифр — римские числа, полноширинные цифры, вульгарная дробь и сравнительная величина словом проходят и в непроверяемый ярус, и, что хуже, в ПОЗИЦИОННУЮ строку `T1`, которая несёт вердикт клаузы спеки 1 под `PASS`: «recall ５６ из １３２» опубликовано, ни к какому ключу не привязано, шесть механизмов молчат, 504 теста зелены.*

*Обе находки — не новые классы. Это пятая форма атаки на ярус и вторая на «истину, посчитанную проверяемой функцией», и обе получены применением F-0048 к механизмам, написанным в том же коммите. Процедура работает; её просто снова не применили к себе.)*
...
$ git rev-parse HEAD
bf55862df0a6aef06e9df4ae9bb2ff602602e135

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic bf55862 [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**HEAD сдвинулся во время моей работы, и это не я.** На входе `047d0dc`, дерево чистое. На выходе `bf55862` — `C224 M4.5(governance): REVIEW_M4_5 addendum 6 - VERDICT ACCEPT WITH CONDITIONS`, изменён ровно `docs/REVIEW_M4_5.md`. `git diff --stat 047d0dc..bf55862 -- crates/ configs/ .github/ Cargo.toml Cargo.lock` — **пусто**, поэтому все измерения выше, сделанные в клоне на `047d0dc`, описывают и `bf55862`.

Из основного дерева я ничего не правил, не коммитил и не пушил. Все замеры — в клоне `…/scratchpad/rt6-c93a5f` с `CARGO_TARGET_DIR = …/scratchpad/tgt-rt6-c93a5f`; клон на выходе был на `047d0dc` с пустым `git status`, чужих правок в нём не появилось. Клон, target и промежуточные файлы удалены; чужое содержимое общего scratchpad не трогалось. `git worktree prune` не запускался ни разу.
...
# ADDENDUM 6 — RED TEAM M4.5, дельта-6 (C226)

> Addendum публикуется **дословно**. Подписанный текст и addendum 1–5 выше не изменены ни в одной строке.
...
$ git rev-parse HEAD
4f8bd9fb6a5829bae927592fc4a9bac263966345

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic 4f8bd9f [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**Изоляция.** Клон `…/scratchpad/rt7-e21b84`, `CARGO_TARGET_DIR = …/scratchpad/tgt-rt7-e21b84`; имена проверены свободными перед созданием. `rt5`/`rt6` не переиспользовались. `git worktree prune` не запускался. Клон, target и шим удалены. Донорские исходники не открывались (D-3).

**База на `4f8bd9f`:** `cargo test --release --workspace` 504 passed / 0 failed; hygiene 11; doc_claims 6; гейт-файл дельтой-6 не тронут.

---

## §0.1. Ошибка в моём отчёте, которую нашёл не я

Дважды — в addendum 4 (§4 п. 1) и в addendum 5 (§7 п. 1) — я опубликовал RT45-A26 как «не воспроизводится на моей платформе», и оба раза это было утверждение о **приборе**, поданное как утверждение о **мире**. Причину назвал governor: `Command::new("git")` на Windows резолвит только `.exe`; я пробовал безрасширенный shell-скрипт, затем скрипт, делегирующий на `/usr/bin/git`, которого в этом окружении нет, — и остановился на отрицательном результате, не построив позитивного контроля, показывающего, что мой прибор вообще способен сработать.

Это ровно мета-правило M-4 и ровно F-0051, и я нарушил его в том же отчёте, где применял его к чужой работе. Второй раз за милестоун отрицательный результат оказался отказом прибора (первый — `403 rate limit`), и второй раз это стоило дельты.

**Я перепроверил сам, десятистрочным шимом на Rust, собранным в `git.exe`:**

```text
$ sed -i 's/^gate_min_recall_arms = 20$/gate_min_recall_arms = 1000/' configs/GATES_V1.toml

(1) правленый файл, бинарь НЕ пересобран, без шима
    checked against: the copy compiled into this binary        EXIT=2   ← отказ
(2) пересборка (как делает `cargo run` в CI), без шима
    checked against: git show                                  EXIT=2   ← отказ
(3) тот же пересобранный бинарь, шим ПЕРВЫМ в PATH, --scope full
    [NOT MET] GT-equivalent topology present in envelope       EXIT=1   ← загрузка ПРОШЛА,
                                                                          порог 1000 доехал до строки
(4) ПОЗИТИВНЫЙ КОНТРОЛЬ: чистое дерево, тот же шим в PATH
    [MET] GT-equivalent topology present in envelope                    ← шим сам по себе ничего не ломает
```

Гипотеза верна, атака работает, `load_for_a_gate_decision` обходится одним шимом. Приоритет находки принадлежит холодному рецензенту; я подтверждаю её собственным исполнением и признаю свою ошибку как ошибку метода, а не как невезение.

---

## §1. Состояние прежних атак

| # | что атаковалось | вердикт на `4f8bd9f` |
|---|---|---|
| A1, A2, A9, A10, A12, A13, A14, A15, A16, A17, A18, A20, A21, A22 | — | **ОТБИТЫ** |
| **RT45-A24** | паддинг вне `envelope_classes` | **ОТБИТА** |
| **RT45-A28** | инъекция внутри `envelope_classes` | **ОТБИТА** |
| **RT45-A29** | величина в непроверяемой нотации | **ОТБИТА** (обе процедуры) |
| **RT45-A19 / A25 / A26 / A27** | F-4 / F-1 / F-3 / F-4 | ПРОХОДЯТ — **заморожены, цены перемерены** |

Исполнение, компактно:

```text
A15  2 ошибки типов: expected `&LegalProfile` / `&LegalCell`
A9/A14  3 ошибки `is more private`
A20  «holds out "tiny-skia", which resolves to no RasterProfile variant»
A16  the_report_aggregates_agree_with_the_run_they_came_from ... FAILED
A17  row "| T1 " position 5: the row says 56, but recall_all.hits is 100
A18  no_workflow_redirects_the_gate_file ... FAILED
A24  «publishes [(1,0),(3,1)] and an INDEPENDENT reading … produces [(1,0)]»
```

### RT45-A28 — ОТБИТА

Дословная инъекция внутрь `envelope_classes`:

```text
pair adv/ambiguous/hole-or-not publishes [(1, 0), (3, 1)] and an INDEPENDENT reading
of the same observation produces [(1, 0)]
test result: FAILED. 9 passed; 1 failed
```

### RT45-A29 — ОТБИТА, обе процедуры

(а) непроверяемый ярус `T10d`, (б) позиционная строка `T1` — обе с римскими U+2160, полноширинными U+FF10 и вульгарной дробью:

```text
(а) row "| T10d " carries the UNPARSED numeric character 'Ⅹ' (U+2169) at byte 891
(б) row "| T1 "   carries the UNPARSED numeric character '５' (U+FF15) at byte 1112
```

Критерий «разобрана ли ячейка целиком» — правильная форма: он не перечисляет алфавиты, `is_numeric` покрывает все числовые категории Unicode, и ошибка называет кодовую точку и байт. Это лучший механизм, произведённый милестоуном после печати типом.

---

## §2. Новые атаки

### RT45-A30 — «независимый путь» разделяет с проверяемым ВОСЕМЬ звеньев из девяти — **MAJOR**

**Что заявлено.** Докблок `classes_by_an_independent_path`: «render, canonicalize, analyze, observe, `propose`. Same inputs, same config, **no shared wrapper** — the same shape as `independent.rs` for the GT signature, and for the same reason».

**Чем он на самом деле является.** Транскрипцией того же вызова. Ниже обёртки `envelope_classes` не переписано НИЧЕГО:

| звено | `envelope_classes` | «независимый путь» |
|---|---|---|
| рендер | `render_cell(scene, cell, 2)` | `render_cell(scene, cell, 2)` |
| канонизация | `CanonicalImage::from_straight_srgb8` | то же |
| анализ | `analyze_full(&img, &ANALYSIS_CONFIG_V1, None)` | то же |
| наблюдение | `observations_for(&ev, "ambiguity")` | то же |
| конверт | `propose(…, &TOPOLOGY_CONFIG_V1)` | то же |
| классы | `envelope.signature_classes()` | то же |

Различается ровно одно: имя функции, в которой эта последовательность записана. Сравнение с `independent.rs` неверно по существу: там переписан АЛГОРИТМ (bit-quad характеристика Эйлера против union-find), здесь переписан вызов.

**Процедура воспроизведения.** Та же инъекция на одно звено ниже — в `Envelope::signature_classes`, который зовут ОБА пути:

```diff
// crates/vice-topology/src/envelope.rs
     pub fn signature_classes(&self) -> BTreeSet<(u32, u32)> {
-        self.hypotheses.iter().map(|h| (h.signature.components, h.signature.holes)).collect()
+        let mut out: BTreeSet<(u32, u32)> =
+            self.hypotheses.iter().map(|h| (h.signature.components, h.signature.holes)).collect();
+        if out.len() == 1 {
+            out.insert((3, 1));
+        }
...
$ gt-corpus topology-check --report docs/gt/TOPOLOGY_M4_5.json
  topology report did NOT reproduce           EXIT=1     ← единственный свидетель
```

Класс `(3, 1)`, которого конверт не производил, опубликован; порог `gate_min_classes_per_retaining_pair` сравнивается с ним; страж, написанный ровно чтобы это поймать, зелёный. Заметил снова только замороженный артефакт — то есть число, а не свойство, и он делал это до дельты-5, до дельты-6 и без них.

**Общее правило класса.** Независимость измеряется не именем функции, а длиной общего суффикса цепи. Две реализации, вызывающие одни и те же восемь функций, детектируют ровно одно место — обёртку, которой они не делятся. Форма, которая работает, в этом проекте уже есть и названа в самом докблоке: `independent.rs` не повторяет вызовы, он считает ту же величину ДРУГОЙ математикой. Здесь это выразимо: классы конверта можно получить прямым перебором пороговых разметок и подсчётом компонент/дыр независимой цепью — той самой, что уже написана.

---

### RT45-A31 — разбор «всей ячейки» применён к одной ячейке из пяти — **MAJOR**

**Что заявлено.** Комментарий механизма: «the criterion is not "does this contain a digit" but **"is the whole cell accounted for"**. Every character of the evidence column must be one this file can classify».

Второе предложение — и есть находка: критерий назван «вся ячейка», а исполнен над `cells[4]`. Колонки `#`, **Gate** и **Статус** не читает ни разбор, ни ярусная проверка, ни `row_numbers`.

**Процедура воспроизведения.** Обычные ASCII-цифры, никакой экзотики; строка `T1` — та, что помечена «клауза спеки 1» и несёт `PASS`:

```diff
--- docs/STATUS_M4_5.md
-| T1 | **GT-equivalent topology present in envelope** (клауза спеки 1) | PASS | scope: …
+| T1 | **GT-equivalent topology present in envelope** (клауза спеки 1) — recall 56 из 132, нетривиальных 8 из 31 | PASS (56/132) | scope: …
```
...
Ложное измерение стоит в НАЗВАНИИ гейта и в колонке СТАТУС строки, выносящей вердикт клаузы спеки 1, и все шесть механизмов молчат.

**Это шестая форма одной атаки (A4 → A11 → A17 → A22 → A29 → A31) и самая дешёвая из шести:** не нужно ни понижать ярус, ни переименовывать строку, ни добавлять приманку, ни искать нотацию — достаточно сдвинуть предложение на одну ячейку влево.

**Общее правило класса.** Пять раз чинили ЯРУС, шестой раз починили НОТАЦИЮ, и оба раза область осталась местом. Критерий, названный «вся ячейка», обязан быть применён ко всем ячейкам строки: `#` — идентификатор, `Gate` и `Статус` — проза и вердикт, и величин в них быть не должно вовсе; `Evidence` — привязанные токены. Это то же правило, что уже написано, применённое к строке, а не к одной её колонке.

---

## §3. Объявленный предел §17: проверен ИЗМЕРЕНИЕМ и подтверждён

Мандат просил проверить, действительно ли `XXII` из ASCII-букв неотделим от прозы, или предел объявлен шире, чем есть. Проверял так: дискриминатор «токен целиком из `IVXLCDM`, длиной ≥ 2, разбирающийся в КАНОНИЧЕСКОЕ римское число», прогнанный по всем ячейкам таблиц документов под механизмом.

```text
table cells scanned : 2554
canonical Roman-numeral tokens of length >= 2 found: 11
   ('docs/STATUS_M4_5.md',            'CI',  '**Условия 26, 27, 29 и прибор CI**')
   ('docs/STATUS_M4_5.md',            'CI',  '`--locked` в CI')
   ('REQUIREMENTS_TRACEABILITY.md',   'CLI', '`oracle::crime::InverseCrime` …')
   ('REQUIREMENTS_TRACEABILITY.md',   'CI',  '**A7.2**: цифры закоммиченных артефактов …')
   … ещё семь, все 'CI'
```

**Одиннадцать попаданий, одиннадцать ложных.** `CI` = 101 и `CLI` = 151 — канонические римские числа и одновременно обычные аббревиатуры, которыми эти документы полны. Дискриминатор срабатывает исключительно на честной прозе, то есть дискриминатором не является; отсечь его можно только словарём исключений, а словарь — ровно та литеральная форма, которую запрещает F-0048.

**Предел объявлен точно.** Занижения нет, завышения нет. Это второй раз в этом милестоуне, когда я проверял объявленное ограничение и оно подтвердилось.

---

## §4. Цены замороженных пунктов, перемерены после переписывания

| # | цена в §17 | моя проверка | вердикт |
|---|---|---|---|
| **F-1** | **одна** строка `run:` (переписано с двух) | подтверждаю исполнением: шим в PATH, коммит не нужен; см. §0.1 | **точна** |
| **F-2** | шаг «измеряет» может не собраться | `cargo run --locked` — 22 измеряющих шага, `cargo build` отдельным шагом — 0 | **точна** |
| **F-3** | один шим в `PATH` | **воспроизведена мной**, с позитивным контролем | **точна** |
| **F-4** | новый `pub(crate)` продуктовый тип + одна `pub fn` | перемерил дословно: 63 рендера, 7 sealed-audit семейств, **258 048 байт**, hygiene 11/11, clippy 0 | **точна** |
| **F-5** | значения гейт-файла не решают ни одной строки §28 M3; файл входит только хешем | это ровно то, что я измерил в addendum 5 (подделка `459→1`, `0.99→0.10` сдвинула одно поле — хеш) | **точна, и ложная формулировка исправлена** |

Ни одна из пяти не занижена. F-1 была ЗАВЫШЕНА и переписана вниз — автор говорит прямо, что завысил её **потому, что считал F-3 непроверяемым**, то есть цена одного остатка зависела от статуса соседнего. Это верное наблюдение и оно моё в той же мере: непроверенным F-3 сделал я.

---

## §5. F-0048 по механизмам дельты-6

| механизм | Q1: литерал-перечень? | Q2: следующая находка | Q3: судья | вердикт |
|---|---|---|---|---|
| `classes_by_an_independent_path` | нет | — | **транскрипция той же цепи; общий суффикс — 8 звеньев** | **не проходит по Q3** (RT45-A30) |
| разбор ячейки (`is_numeric` + catch-all) | **нет** — критерий, а не алфавит | «изменить критерий» | классификация всех символов | **проходит по Q1–Q3** |
| …его ОБЛАСТЬ (`cells[4]`) | **да** — одна колонка из пяти | дописать колонку | место | **не проходит** (RT45-A31) |

Из двух новых механизмов один — правильной формы и с неправильной областью, второй — правильной области и неправильной формы. Q5 (двусторонность) соблюдён у обоих: `checked >= 2` у независимого пути, и разбор ячейки проверен в обе стороны моими двумя процедурами.

---

## §6. Атаки, которые НЕ удались

**FA.** RT45-A28 и RT45-A29 (обе процедуры) — отбиты, с точными сообщениями.
**FB.** Четырнадцать прежних атак — отбиты, регрессий нет.
**FC.** Отделить `XXII` от прозы механически — **не удалось**, 11/11 ложных срабатываний; предел объявлен верно.
**FD.** Заставить `classes_by_an_independent_path` вернуть не то же самое на одном входе (недетерминизм) — не удалось: цепь чистая.
**FE.** `*.yaml` в обход `no_workflow_redirects_the_gate_file` — закрыто в этой дельте; расширение проверяется в обоих написаниях.
**FF.** Артефакт, детерминизм, §27.6 — без изменений; `crates/vice-topology/` дельтой-6 не тронут.

---

## §7. Что я не смог атаковать

1. **CI прогоном** — сеть не использовалась, run 70 не читал. Шим я проверил локально; поведение GitHub-раннера — вне досягаемости.
2. **Свип по бюджету конверта** — не сделан.
3. **Донорские исходники** — не открывались (D-3).
4. **Пять замороженных измерений `--ignored`** — выборочно.
5. **`achievable` внутри `SupportedModelUniverse`** — владелец M6/M7.

---

## §8. RED TEAM VERDICT (addendum 6)

**RED TEAM VERDICT (addendum 6): FAIL**

*(Дельта-6 закрыла обе предъявленные находки по-настоящему: RT45-A28 роняет тест с сообщением, называющим пару и оба чтения; RT45-A29 роняет тест на кодовой точке и байте, обеими моими процедурами, и критерий «разобрана ли ячейка целиком» — правильной формы, не перечень алфавитов, лучший механизм милестоуна после печати типом. Объявленный предел §17 я проверил измерением и подтверждаю: `XXII` из ASCII-букв неотделим — 11 попаданий, 11 ложных, все `CI` и `CLI`. Все пять замороженных цен точны; F-1 переписана ВНИЗ, и причина названа верно.*

*Основание FAIL — два MAJOR. RT45-A30: `classes_by_an_independent_path` объявлен «no shared wrapper — the same shape as `independent.rs`», но ниже обёртки он разделяет с проверяемым все восемь звеньев — `render_cell`, канонизацию, `analyze_full`, `ANALYSIS_CONFIG_V1`, `observations_for`, `propose`, `TOPOLOGY_CONFIG_V1`, `signature_classes`; инъекция в `Envelope::signature_classes` невидима, десять тестов топологии зелены, клауза 2 MET, 504 зелёных, замечает опять только артефакт. `independent.rs` переписывает АЛГОРИТМ; это переписывает вызов, и сравнение с ним неверно. RT45-A31: критерий назван «вся ячейка», а исполнен над колонкой Evidence — обычные ASCII-цифры в колонках Gate и Статус строки `T1`, несущей вердикт клаузы спеки 1 под PASS, проходят все шесть механизмов; это шестая форма одной атаки и самая дешёвая из шести.*

*И то, что я обязан поставить рядом с вердиктом, а не под ним: в этой же дельте моя собственная ошибка метода стоила проекта дороже, чем любая из двух находок. Дважды опубликовать «не воспроизводится» без позитивного контроля — это F-0051, написанный про меня, и завышенная цена F-1 держалась на моём отрицательном результате. Вердикт остаётся FAIL по механизмам; репутационно эта дельта — не в пользу red team.)*
...
$ git rev-parse HEAD
4f8bd9fb6a5829bae927592fc4a9bac263966345        (не двигался)
...
HEAD не двигался; рабочее дерево чистое на входе и на выходе; из основного дерева ничего не правилось, не коммитилось и не пушилось. Все замеры — в клоне `…/scratchpad/rt7-e21b84` с `CARGO_TARGET_DIR = …/scratchpad/tgt-rt7-e21b84`; клон на выходе был на `4f8bd9f` с пустым `git status`, чужих правок в нём не появилось. Клон, target, шим (`…/scratchpad/shim7`, включая `git.exe`) и промежуточные json удалены — шим стёрт намеренно и называю это вслух, потому что оставленный в общем каталоге поддельный `git.exe` был бы ровно тем инцидентом, который этот проект уже переживал. Чужое содержимое общего scratchpad не трогалось. `git worktree prune` не запускался ни разу.
...
# ADDENDUM 7 — RED TEAM M4.5, дельта-7 (C229)

> Addendum публикуется **дословно**. Подписанный текст и addendum 1–6 выше не изменены ни в одной строке.
...
$ git rev-parse HEAD
b75f41cb5e8070a725a0eb8b247baee2cb38bfa6

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic b75f41c [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**Изоляция.** Клон `…/scratchpad/rt8-d47c19`, `CARGO_TARGET_DIR = …/scratchpad/tgt-rt8-d47c19`; имена проверены свободными. `rt7`/`rt6`/`rt5` не переиспользовались. `git worktree prune` не запускался. Клон и target удалены. Донорские исходники не открывались (D-3).

**База на `b75f41c`:** `cargo test --release --workspace` 504 passed / 0 failed; hygiene 11; doc_claims 6; `fmt` чисто; clippy 0; гейт-файл дельтой-7 не тронут.

---

## §1. Состояние прежних атак

| # | что атаковалось | вердикт на `b75f41c` |
|---|---|---|
| A1, A2, A9, A10, A12–A18, A20–A22, A24, A28, A29 | — | **ОТБИТЫ** |
| **RT45-A30** | инъекция в `Envelope::signature_classes` | **ОТБИТА** |
| **RT45-A31** | величина вне колонки Evidence | **ОТБИТА, обе половины** |
| **RT45-A19/A25/A26/A27** | F-4 / F-1 / F-3 / F-4 | ПРОХОДЯТ — заморожены |

### RT45-A30 — ОТБИТА

Инъекция дословно, в `Envelope::signature_classes`:

```text
pair adv/ambiguous/hole-or-not publishes the class (3, 1), which NO threshold of its own
observation produces
test result: FAILED. 12 passed; 1 failed
```

`signature_classes` больше не на пути проверки — по построению, а не по договорённости. Это правильный ответ, и он сработал.

### RT45-A31 — ОТБИТА, обе половины порознь

```text
величина в колонке 2:  row "| T1 " carries the quantity ["56","132","8","31"] in column 2.
                       Only the evidence column is read by the positional binding …
неразобранный символ
в колонке 3:           row "| T1 " carries the UNPARSED numeric character 'Ⅹ' (U+2169)
                       at byte 5 of column 3 … "is the whole ROW accounted for"
```

Область стала строкой. Обе половины имеют собственное срабатывание — то, чего требовало M45-N28.

### Прочие, компактно

```text
A15  2 ошибки типов (&LegalProfile / &LegalCell)        A16  aggregates_agree ... FAILED
A20  «resolves to no RasterProfile variant»             A17  row "| T1 " position 5: says 56, is 100
A18  no_workflow_redirects_the_gate_file ... FAILED     A2   independent_chain_agrees ... FAILED
```
...
### RT45-A32 — судья один, ролей две, а его собственный свидетель-набор — двенадцать фикстур не крупнее 7×7 — **MAJOR**

**Что заявлено.** Новый путь «переписывает АЛГОРИТМ»: пороговые разметки, посчитанные `independent::signature_of`, «которая есть уже написанная цепь для GT-сигнатуры». Общих звеньев названо четыре.

**Что из этого следует, и это не сказано.** `independent::signature_of` теперь судья ДВУХ ролей: истина клаузы 1 (`topology/mod.rs:396`) и достижимость классов клаузы 2 (`tests_envelope.rs`). А доказан он одним тестом — `the_independent_chain_agrees_with_the_production_signature`, — который ходит по **двенадцати рукописным фикстурам**:

```text
3x3, 3x3, 5x5, 5x5, 2x2, 5x3, 4x4, 2x2, 2x2, 4x4, 7x7  (+ диагональное кольцо)
наибольший размер в свидетель-наборе судьи: 7
```

Поля, которые он судит, — рендеры на `collapse_cell` размера **16 и 128**, и arm-ы корпуса 32–512 px. Между свидетелем и предметом лежит порядок величины, и это мета-правило M-2 дословно: фикстур принадлежит подклассу, в котором дефект недостижим.

**Процедура воспроизведения.** Одна правка, в судье, срабатывающая только там, куда его свидетель-набор не достаёт:

```diff
// crates/vice-bench/src/topology/independent.rs, fn signature_of
     let comps = components(inside, width_px, height_px, conn.foreground());
     let x = euler(inside, width_px, height_px, conn.foreground());
+    let lit = inside.iter().filter(|b| **b).count();
+    if width_px > 8 && lit == inside.len() {
+        return IndependentSignature { components: 3, holes: 1, euler: 2 };
+    }
     IndependentSignature {
```

Условие — разметка «всё внутри», которую даёт уровень 0 любого поля покрытия, на сетке крупнее любой фикстуры кросс-чека.

**Результат правки СУДЬИ, и больше ничего:**
...
$ cargo fmt --all --check                                FMT CLEAN
$ cargo clippy --workspace --all-targets -- -D warnings   0 диагностик
$ cargo test --release -p vice-bench the_independent_chain_agrees
  ... ok                                                             ← его собственный кросс-чек
```

Судья, отвечающий за истину клаузы 1 и за проверку клаузы 2, неверен на КАЖДОЙ сетке крупнее 7×7 — и не замечает НИЧЕГО. Ни один из 504 тестов, ни артефакт, ни clippy.

**Плюс одна строка — и механизм дельты-7 принимает выдуманный класс:**
...
$ cargo test --release -p vice-bench the_independent_chain_agrees
  ... ok
$ cargo test --release -p vice-bench every_published_class
  ... ok                                                   ← механизм, построенный чтобы это ловить
$ gt-corpus topology --scope full
  [MET] GT-equivalent topology present in envelope
  [MET] ambiguous fixtures retain alternatives
  [MET] no magic-threshold-only architecture
...
Удерживающая пара несёт ОДИН настоящий класс и один выдуманный; `gate_min_classes_per_retaining_pair = 2` удовлетворён; все три клаузы MET; всё зелено. Только `topology-check` краснеет — на изменившемся числе, как всегда, и только из-за второй правки.

**Почему это MAJOR, а не MINOR.** Правка судьи в одиночку не переворачивает сегодня ни одной клаузы — но она делает неработающим ровно тот механизм, ради которого написана дельта-7, и делает это молча. Дельта-6 разделяла с проверяемым восемь звеньев; дельта-7 честно свела их к четырём, но перенесла вес на пятое — на судью, чья правильность нигде не измерена в области, где он применяется.

**Общее правило класса.** Судья, доказанный рукописным свидетель-набором, доказан на этом наборе. Если он судит поля 16–512 px, набор обязан их содержать, либо доказательство обязано быть не набором: дифференциальный прогон против продакшн-сигнатуры на СЛУЧАЙНЫХ разметках тех размеров, которые встречаются в корпусе. Такой прогон в этом проекте уже делался — 1 115 536 сравнений в addendum 2, — но он был МОИМ пробником и в дерево не попал. Собственный кросс-чек проекта — двадцать четыре сравнения.

---

### RT45-A33 — «четыре общих звена» их пять — **MINOR**

Названы `render_cell`, канонический образ, `analyze_full`, `ANALYSIS_CONFIG_V1`. Пятое:

```text
crates/vice-topology/src/lib.rs:175            for conn in ComplementaryConnectivity::arms()   ← продакшн
crates/vice-bench/src/topology/tests_envelope.rs:119  for conn in ComplementaryConnectivity::arms()   ← «независимый» путь
```

Обе стороны берут множество конвенций из одной функции. Направление безвредно — сужение `arms()` делает `reachable` МЕНЬШЕ, то есть проверку строже, и RT45-A1 это место уже закрывает отдельно. Но число «четыре» подано как измеренный факт в доккомментарии, и оно неверно. Записываю MINOR: не дыра, а неточность в том самом числе, которое автор назвал «the point».

---

## §3. Атаки, которые НЕ удались (измерение прочности)

**FA. Ослабление отношения: равенство → вложение.** Мандат просил найти, что теперь проходит. Измерил ширину допуска:

```text
adv/ambiguous/hole-or-not  scene0: published 1 [(1,0)]        | reachable 2 | допуск 1  → {(0,0)}
adv/ambiguous/hole-or-not  scene1: published 1 [(1,0)]        | reachable 2 | допуск 1  → {(0,0)}
adv/ambiguous/bridge-or-gap scene0: published 2 [(1,0),(2,0)] | reachable 2 | допуск 0
adv/ambiguous/bridge-or-gap scene1: published 2 [(1,0),(2,0)] | reachable 2 | допуск 0
```

На паре, которая НЕСЁТ порог (`bridge-or-gap`), вложение совпадает с равенством: допуска нет вовсе, подставить нечего. Единственный допущенный класс — вырожденный `(0,0)` у `hole-or-not`, а та пара не удерживающая, и попытка добить им удерживающую роняет тест:

```text
pair adv/ambiguous/bridge-or-gap publishes the class (0, 0), which NO threshold of its own
observation produces
```

Ослабление отношения на этом корпусе неэксплуатируемо. Измерено, не оценено.

**FB. Инъекция в четыре названных общих звена.** Меняет саму наблюдаемую картину, и это ловит артефакт — ровно как сказано в доккомментарии. Отдельной находки нет.

**FC. Недетерминизм нового пути.** Повторный вызов даёт то же самое; цепь чистая.

**FD. Четырнадцать прежних атак** (§1) — отбиты, регрессий нет.

---

## §4. F-0048 по механизмам дельты-7

| механизм | Q1: литерал-перечень? | Q2: следующая находка | Q3: судья | вердикт |
|---|---|---|---|---|
| `classes_reachable_by_thresholding` | нет — 257 уровней × 2 конвенции, не список | «изменить критерий» | **`independent::signature_of`, доказанный 12 фикстурами ≤7×7** | **не проходит по Q3** (RT45-A32) |
| разбор ВСЕЙ СТРОКИ (колонки 1–5) | нет — критерий, а не алфавит и не колонка | — | классификация всех символов всех ячеек | **проходит Q1–Q3, обе половины проверены порознь** |

Один из двух проходит полностью — впервые за семь дельт механизм проходит собственный тест автора без оговорок. Второй проходит Q1 и Q2 и падает на Q3, и падает не потому, что судья плох, а потому, что область его доказательства меньше области его применения.

---

## §5. Ответ на вторую половину F-0053 — про мой собственный метод

Запись говорит: *механизм, доказанный только ручным нокаутом, не защищён ничем; нокаут показывает, что он сработал однажды, и ничего не говорит про завтра.* Мандат спрашивает, чем защищены мои нокауты. Отвечаю прямо, потому что ответ неприятный.

**Ничем.** Все тридцать три атаки этого отчёта — ручные, одноразовые и живут в клоне, который я удаляю в конце каждой дельты. Их воспроизводимость обеспечена только тем, что процедура выписана в тексте; переезд механизма ломает процедуру молча, и обнаруживается это тем, что я в следующей дельте выполняю её заново вручную. Дважды это уже стоило дорого: RT45-A26 я объявил невоспроизводимой, потому что мой прибор не умел того, что я от него хотел, и не построил позитивного контроля (§0.1 addendum 6). Это ровно тот же дефект, что RT45-A32: **инструмент проверен на подклассе, в котором дефект недостижим**, — только инструментом был я.

Что из этого следует практически, и это моя рекомендация, а не находка: **атаки red team должны жить в дереве как `#[ignore]`-тесты, каждая со своим позитивным контролем** — «примени правку, механизм обязан покраснеть; сними правку, обязан позеленеть». Тогда переезд механизма ломает атаку громко, а не молча, и «отбита» перестаёт быть утверждением о прошлом. Пока этого нет, каждая строка «ОТБИТА» в моих таблицах — измерение того дня, и не более.

---

## §6. Что я не смог атаковать

1. **CI прогоном** — сеть не использовалась, run 73 не читал.
2. **Свип по бюджету конверта** — не сделан седьмую дельту подряд.
3. **Донорские исходники** — не открывались (D-3).
4. **Пять замороженных измерений `--ignored`** — выборочно.
5. **`achievable` внутри `SupportedModelUniverse`** — владелец M6/M7.
6. **Дифференциальный прогон судьи на корпусных размерах** — я его НЕ делал в этой дельте; я показал, что его нет, а не что судья неверен. Судья, насколько я мерил, верен; незащищён — да.

---

## §7. RED TEAM VERDICT (addendum 7)

**RED TEAM VERDICT (addendum 7): FAIL**

*(Дельта-7 — лучшая работа милестоуна по форме. RT45-A30 закрыт настоящим переписыванием алгоритма: `propose`, `signature_classes`, `observations_for` и `TOPOLOGY_CONFIG_V1` ушли с пути, инъекция в `Envelope::signature_classes` теперь роняет тест по построению, а не по договорённости. RT45-A31 закрыт по всей СТРОКЕ, и обе половины срабатывают порознь. Ослабление равенства до вложения я атаковал и не смог: на паре, несущей порог, допуск равен нулю — измерено, а не принято на слово. Разбор всей строки — первый механизм за семь дельт, проходящий F-0048 без оговорок.*

*Основание FAIL — RT45-A32 (MAJOR). Дельта-7 честно свела общие звенья с восьми до четырёх и перенесла вес проверки на пятое, о котором не сказала: `independent::signature_of` стал судьёй двух ролей сразу — истины клаузы 1 и достижимости классов клаузы 2, — а доказан он двенадцатью рукописными фикстурами, крупнейшая 7×7, при том что судит поля 16 и 128 px. Одна правка в судье, срабатывающая только выше 7×7, оставляет 504 теста зелёными, артефакт побайтово тем же, `fmt` чистым, clippy молчащим и его собственный кросс-чек зелёным; ещё одна строка — и `every_published_class_came_from_the_envelope` принимает выдуманный класс, а `gate_min_classes_per_retaining_pair` удовлетворяется одним настоящим чтением и одним фабрикованным при трёх MET. Это мета-правило M-2 дословно, применённое к судье. Плюс MINOR RT45-A33: общих звеньев пять, а не четыре — `ComplementaryConnectivity::arms()` разделяют обе стороны; направление безвредно, но число названо фактом и неверно.*

*И то, что я обязан поставить рядом, а не под вердиктом: F-0053 бьёт по мне сильнее, чем по автору. Мои тридцать три нокаута не защищены ничем, живут в удаляемых клонах и проверяются тем, что я выполняю их руками; дважды это уже дало ложный отрицательный результат. Пока атаки red team не переехали в дерево с позитивными контролями, слово «ОТБИТА» в этих таблицах — про сегодня.)*
...
$ git rev-parse HEAD
5afc2465693546e5450ac30799fb80a457f7883d

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic 5afc246 [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**HEAD сдвинулся во время моей работы, и это не я.** На входе `b75f41c`, дерево чистое. На выходе `5afc246` — `C230 M4.5(governance): REVIEW_M4_5 addendum 8 - VERDICT ACCEPT WITH CONDITIONS`, изменён ровно `docs/REVIEW_M4_5.md`. `git diff --stat b75f41c..5afc246 -- crates/ configs/ .github/ Cargo.toml Cargo.lock` — **пусто**, поэтому все измерения выше, сделанные в клоне на `b75f41c`, описывают и `5afc246`.

Из основного дерева я ничего не правил, не коммитил и не пушил. Все замеры — в клоне `…/scratchpad/rt8-d47c19` с `CARGO_TARGET_DIR = …/scratchpad/tgt-rt8-d47c19`; клон на выходе был на `b75f41c` с пустым `git status`, чужих правок в нём не появилось. Клон, target и промежуточные json удалены; чужое содержимое общего scratchpad не трогалось. `git worktree prune` не запускался ни разу.
...
# ADDENDUM 8 — RED TEAM M4.5, дельта-8 (C232)

> Addendum публикуется **дословно**. Подписанный текст и addendum 1–7 выше не изменены ни в одной строке.
...
$ git rev-parse HEAD
bed91f216a2e12aaa723f6f69a166707064c8bb8

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic bed91f2 [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**Изоляция.** Клон `…/scratchpad/rt9-b62f38`, `CARGO_TARGET_DIR = …/scratchpad/tgt-rt9-b62f38`; имена проверены свободными. `rt8`/`rt7`/`rt6`/`rt5` не переиспользовались. `git worktree prune` не запускался. Клон и target удалены. Донорские исходники не открывались (D-3).

**База на `bed91f2`:** 506 passed / 0 failed; hygiene 12; doc_claims 6; гейт-файл дельтой-8 не тронут.

---

## §1. Состояние прежних атак — пятнадцать отбитых, все проверены исполнением

```text
A15  2 ошибки типов (&LegalProfile / &LegalCell)          A16  aggregates_agree ... FAILED
A9/A14  3 ошибки «is more private»                        A17  row "| T1 " position 5: says 56, is 100
A20  «resolves to no RasterProfile variant»               A18  no_workflow_redirects ... FAILED
A2   independent_chain_agrees ... FAILED                  A31  «carries the quantity … in column 2»
A30  «which NO threshold of its own observation produces»
```

Плюс A1, A10, A12, A13, A21, A22, A24, A28, A29 — отбиты (проверены в дельтах 5–7, код на этих путях дельтой-8 не двигался).

| # | вердикт на `bed91f2` |
|---|---|
| **RT45-A32** | **ПО-ПРЕЖНЕМУ ПРОХОДИТ ДОСЛОВНО** — см. §2 |
| **RT45-A33** | **ОТБИТА ЧАСТИЧНО** — пересчёт ближе к правде, но список неполон; моё число ниже |
| остальные пятнадцать | **ОТБИТЫ** |

---

## §2. RT45-A32 — моя правка проходит дословно; дифференциальный прогон закрыл разрыв по РАЗМЕРУ и оставил разрыв по ПЛОТНОСТИ — **MAJOR**

**Что заявлено.** 400 сравнений на 9/16/32/64/128 px, обе конвенции, четыре режима плотности, против прежних двадцати четырёх. Governor передал, что моя правка теперь ловится.

**Ловится не моя.** Опубликованная в addendum 7 правка триггерится не по размеру, а по ПЛОТНОСТИ разметки:
...
Условие — разметка «всё внутри», то есть плотность **1.000**. Сэмплер берёт четыре режима:

```text
регима     u64::MAX/8: ожидаемая плотность 0.125; P(all-true на 9x9) = 7.1e-74
регима     u64::MAX/3:                      0.333;                     2.3e-39
регима     u64::MAX/2:                      0.500;                     4.1e-25
регима   3*u64::MAX/4:                      0.750;                     7.6e-11
```

Плотность 1.0 не производится ни одним из четырёх режимов ни на одном из пяти размеров. А конвейер подаёт судье именно её: `classes_reachable_by_thresholding` идёт по `step in 0..=256`, и уровень 0 даёт разметку «всё внутри» для КАЖДОГО поля.

**Процедура воспроизведения — правка скопирована из addendum 7 без единого изменения.**

```text
$ cargo test --release -p vice-bench the_independent_chain_agrees
  ... ok                                        ← рукописный кросс-чек (12 фикстур, ≤7×7)
$ cargo test --release -p vice-bench the_independent_judge_agrees_with_production_at_corpus_sizes
  ... ok                                        ← НОВЫЙ дифференциальный прогон, 400 сравнений
$ cargo test --release --workspace              506 passed / 0 failed
$ gt-corpus topology-check --report docs/gt/TOPOLOGY_M4_5.json
  topology report reproduced with every metric compared   EXIT=0   ← артефакт побайтово тот же
```

**Плюс та же одна строка паддинга, и механизм дельты-7 снова принимает выдуманный класс:**

```text
$ cargo test --release -p vice-bench every_published_class_came_from_the_envelope   ... ok
$ cargo test --release -p vice-bench the_independent_judge_agrees_with_production   ... ok
$ gt-corpus topology --scope full
  [MET] GT-equivalent topology present in envelope
  [MET] ambiguous fixtures retain alternatives
  [MET] no magic-threshold-only architecture
...
**Что дельта-8 всё же купила, и это надо сказать точно.** Разрыв по РАЗМЕРУ закрыт по-настоящему: правка, триггерящаяся по `width_px`, теперь краснеет, и форма — дифференциальный прогон против продакшн-сигнатуры — правильная, моя собственная из addendum 2, наконец в дереве. Куплена одна из двух осей.

**Общее правило класса.** Выборка описывается тем, чем её параметризовали. Четыре порога плотности — это литеральный перечень (F-0048, вопрос 1), и он покрывает внутренность интервала, оставляя КРАЯ: плотность 0 и плотность 1. А края — не экзотика: это уровень 0 и уровень выше максимума, то есть первая и последняя итерации того самого цикла, ради которого судья и вызывается. Проверяемая замена не требует пятого режима: включить в выборку граничные разметки явно (`all-true`, `all-false`, одиночный пиксель, полная строка/столбец) — они детерминированы, их четыре, и они и есть то, что конвейер гарантированно подаёт.

**Про seed, отдельно.** Фиксированный seed при фиксированных размерах и фиксированном числе попыток даёт не выборку, а **перечень из 400 конкретных разметок**, записанный сжатым видом. Как средство воспроизводимости — правильно. Как аргумент покрытия — нет: множество проверенных входов не растёт ни от прогона к прогону, ни от машины к машине. Честная форма — фиксированный seed ПЛЮС явные граничные случаи, потому что перечень, который нельзя расширить случайностью, обязан быть полным по краям.

---

## §3. RT45-A33 — моё число: **8** при его группировке, **10** без группировки

Автор пересчитал с четырёх на семь и дал полный список. Считаю независимо, по коду.

| # | звено | у автора |
|---|---|---|
| 1 | `render_cell(scene, cell, 2)` — включая аргумент `2` | есть |
| 2 | `CanonicalImage::from_straight_srgb8` | есть |
| 3 | `analyze_full` | есть |
| 4 | `ANALYSIS_CONFIG_V1` | есть |
| 5 | **`out.chosen`** | **НЕТ** |
| 6 | `ev.alpha_field()` | есть (в группе 7) |
| 7 | `ev.width_px()` / `ev.height_px()` | есть (в группе 7) |
| 8 | `ComplementaryConnectivity::arms()` | есть |
| 9 | `conn.foreground()` | есть |

```text
--- production (envelope_classes) ---            --- checker ---
let out = analyze_full(&img, &ANALYSIS_CONFIG_V1, None);   let out = analyze_full(&img, &ANALYSIS_CONFIG_V1, None);
let Some(ev) = out.chosen else { … };                      let ev = out.chosen.expect("a coverage field");
let obs = observations_for(&ev, "ambiguity");              let alpha = ev.alpha_field();
   └─ alpha: ev.alpha_field(), width_px: ev.width_px() …      let (w, h) = (ev.width_px(), ev.height_px());
```

**При группировке автора (аксессоры формы наблюдения = одно звено) правильное число 8, а не 7.** Пропущен `out.chosen` — и пропущен он не безобидно: это не аксессор без арифметики, это ВЫБОР гипотезы среди тех, что вернул `analyze_full`. Он принадлежит первой группе (наблюдение, звенья 1–4), а не второй («аксессоры, сужающие видимость»). Без группировки распадается на 10.

Третий счёт подряд оказывается в пользу автора: 8 «независимо», потом 4, потом 7 — и каждый раз меньше фактического. Направление ошибки устойчиво, и это стоит записать отдельно от самой ошибки.

**Оценка: ОТБИТА ЧАСТИЧНО.** Пятое звено, которое я нашла, добавлено; список стал списком, а не числом; но он всё ещё неполон, и разница ровно та, что меняет вывод — «наблюдение общее в четырёх точках» против «в пяти».

---

## §4. F-7 подтверждён эксплуатацией, по объявленной цене

Автор объявил: проверка существования видит ФРАЗУ, а не поведение. Проверяю.

```text
(A) дерево как закоммичено, workflow `attack.yaml` с `--gates configs/OTHER.toml`:
    no_workflow_redirects_the_gate_file ... FAILED
    «attack.yaml passes --gates configs/OTHER.toml, which is not in GATE_PATHS»   ← ловит

(B) добавлена ОДНА строка выше нетронутой фразы:
        if ext != "yml" { continue; }
        if ext != "yml" && ext != "yaml" { continue; }     ← фраза на месте, grep находит 1
    the_mechanisms_this_milestone_built_are_still_present ... ok      ← существование: зелено
    no_workflow_redirects_the_gate_file                   ... ok      ← поведение: потеряно
    cargo test --release --workspace                      506 passed / 0 failed
```

RT45-A18 переоткрыт при зелёном стороже существования. Это ровно F-7, ровно по объявленной цене, и я подтверждаю его как **объявленное ограничение**, а не как находку.

Отмечу калибровку, потому что она в пользу автора: первая моя попытка (с sidecar-файлом `configs/EVIL.toml`) покраснела — её поймала вторая половина того же стража, обход всего репозитория в поисках файлов с замороженными секциями. Чтобы изолировать `.yaml`-половину, sidecar пришлось убрать. Две половины стража независимы, и это правильно.

---

## §5. F-0048 по механизмам дельты-8

| механизм | Q1: литерал-перечень? | Q2: следующая находка | Q3: судья | вердикт |
|---|---|---|---|---|
| дифференциальный прогон судьи | **да** — 5 размеров, 4 режима плотности, фиксированный seed = перечень из 400 разметок | «дописать режим/размер» | продакшн-сигнатура (настоящий судья) | **не проходит по Q1/Q2** (RT45-A32) |
| `the_mechanisms_…_are_still_present` | **да** — шесть пар (файл, фраза) | «дописать седьмую пару» | текст, не поведение | **не проходит**, и **объявлено** как F-7 |
| условие 44 (потолок опорного множества) | нет — граница выведена из размера поля | — | свойство наблюдения | **проходит** |

Из трёх механизмов дельты-8 один проходит, один не проходит и объявлен, один не проходит и объявлен закрытым.

---

## §6. Атаки, которые НЕ удались

**FA. Разрыв 128–512 px.** Не эксплуатируется сегодня: судья вызывается на `TOPOLOGY_CELL_IDS` (32/64/128) и на `collapse_cell` (16 и 128). Ни одного вызова выше 128 в текущем конвейере нет. Разрыв, который автор называет реальным, реален для `SIZES_PX`, но не достижим — говорю это в его пользу, потому что он назвал его сам и мог бы промолчать.

**FB. Разрыв МЕЖДУ размерами** (например 48, 96). Не эксплуатируется: все ячейки — степени двойки, промежуточных размеров конвейер не производит.

**FC. Условие 44.** Пытался насытить опорное множество, чтобы вложение стало тривиальным: потолок `reachable.len() * 8 < size²` держит — на 16×16 допускается меньше 32 классов, фактически 2.

**FD. Пятнадцать прежних атак** — отбиты (§1).

**FE. Артефакт.** `topology-check` — reproduced, exit 0, включая прогон с правкой судьи.

---

## §7. Ответ на вопрос governor-а — числом

**Сходимость или пора замораживать: из двух моих находок дельты-8 НОВЫХ КЛАССОВ — ноль. Оба — тот же класс на уровень глубже.**

- **RT45-A32** — мета-правило M-2, четвёртый экземпляр подряд в одной цепочке: свидетель принадлежит подклассу, где дефект недостижим. Подкласс менялся — сначала размер фикстур (≤7×7), теперь плотность выборки (0.125…0.75). Класс тот же.
- **RT45-A33** — F-0048, вопрос 1: механизм содержит перечень, и перечень неполон. Тот же класс, что все семь экземпляров F-0048.
- **F-7** — не находка вовсе, а подтверждение объявленного.

Отсюда моё суждение о сходимости, и оно неоднозначное в обе стороны, поэтому скажу обе.

**За заморозку.** Восемь дельт, и глубина растёт монотонно: имена → типы → продукт → область → судья → область доказательства судьи → плотность выборки внутри области доказательства. Каждый уровень дешевле предыдущего в исполнении и дороже в починке. Стоимость атаки в дельте-8 — одна строка в судье плюс одна в `ambiguity.rs`; стоимость правильной починки — переписать выборку. Отношение ухудшается, и это признак, что предмет исчерпан как ПРЕДМЕТ АТАКИ, а не что он стал безопаснее.

**Против заморозки — и это перевешивает для конкретно этой находки.** RT45-A32 не «глубже», он **тот же самый**: правка не изменилась ни на символ между addendum 7 и addendum 8. Механизм, объявленный закрывающим её, её не закрывает. Замораживать остаток целиком — законно; объявлять закрытой находку, которая проходит дословно, — нет. Это разные решения, и первое не требует второго.

**Рекомендация, одной строкой:** заморозить КЛАСС (M-2 применительно к области доказательства инструментов) с владельцем M5, но не закрывать RT45-A32 — она открыта, и её цена измерена.

---

## §8. Что я не смог атаковать

1. **CI прогоном** — сеть не использовалась, run 76 не читал.
2. **Свип по бюджету конверта** — не сделан восьмую дельту подряд.
3. **Донорские исходники** — не открывались (D-3).
4. **Пять замороженных измерений `--ignored`** — выборочно.
5. **`achievable` внутри `SupportedModelUniverse`** — владелец M6/M7.
6. **Собственные атаки в дереве.** Рекомендация из addendum 7 §5 не исполнена — ни автором, ни мной. Все тридцать четыре атаки по-прежнему живут в удаляемых клонах, и RT45-A32 — прямое следствие: я проверила её вручную, и только поэтому заметила, что фикс её не ловит. Автоматизированной она бы покраснела сама.

---

## §9. RED TEAM VERDICT (addendum 8)

**RED TEAM VERDICT (addendum 8): FAIL**

*(Дельта-8 сделала правильную вещь правильной формой: дифференциальный прогон против продакшн-сигнатуры на корпусных размерах — это то, что я просила в addendum 7, и он в дереве, и он покрыл разрыв по РАЗМЕРУ по-настоящему. Условие 44 — годная граница, выведенная из свойства наблюдения. Пересчёт общих звеньев с четырёх до семи — движение в правильную сторону и с полным списком. F-7 объявлен честно и подтверждён мной по объявленной цене. Пятнадцать прежних атак отбиты.*

*Основание FAIL — RT45-A32 проходит ДОСЛОВНО: правка, опубликованная в addendum 7, скопированная без единого изменения, оставляет зелёными и рукописный кросс-чек судьи, и новый дифференциальный прогон из 400 сравнений, и 506 тестов, и побайтово тот же артефакт; плюс одна строка паддинга — и `every_published_class_came_from_the_envelope` принимает выдуманный класс при трёх MET. Причина точна и измерена: мой триггер — плотность 1.000, сэмплер берёт 0.125/0.333/0.500/0.750, вероятность попадания 7.6e-11 в лучшем режиме, а конвейер подаёт судье именно плотность 1.000 на уровне 0 каждого поля. Фикс закрыл одну ось выборки из двух. Плюс MINOR RT45-A33: общих звеньев восемь при группировке автора, не семь; пропущен `out.chosen`, и он принадлежит группе «наблюдение», а не группе «аксессоры» — третий счёт подряд в свою пользу.*

*И ответ на вопрос о сходимости, числом, как просили: новых классов среди моих находок дельты-8 — **ноль из двух**. Оба — прежние классы на уровень глубже. Это аргумент за заморозку класса, но не за объявление закрытой находки, которая проходит дословно.)*
...
$ git rev-parse HEAD
bed91f216a2e12aaa723f6f69a166707064c8bb8        (не двигался)
...
**Рабочее дерево на выходе НЕ чистое, и это не моё.** На входе оно было пустым. `docs/REVIEW_M4_5.md` изменён во время моей работы — по форме это очередной addendum холодного рецензента. Я к нему **не прикасалась и не откатывала**; HEAD не двигался.

Из основного дерева я ничего не правила, не коммитила и не пушила. Все замеры — в клоне `…/scratchpad/rt9-b62f38` с `CARGO_TARGET_DIR = …/scratchpad/tgt-rt9-b62f38`; клон на выходе был на `bed91f2` с пустым `git status`, чужих правок в нём не появилось. Клон, target и промежуточные json удалены; чужое содержимое общего scratchpad не трогалось. `git worktree prune` не запускался ни разу.
...
# ADDENDUM 9 — RED TEAM M4.5, дельта-9 (C235–C236), ФИНАЛЬНЫЙ ПРОХОД

> Addendum публикуется **дословно**. Подписанный текст и addendum 1–8 выше не изменены ни в одной строке.
...
$ git rev-parse HEAD
d5bbc42d360c6ac705fdaf0159b8ef5b9392b7c0

$ git worktree list
C:/Users/nirrt/Toolset/vice-classic d5bbc42 [main]        ← только основное дерево

$ sha256sum /c/Users/nirrt/Downloads/VICE_CLASSIC_CORE_AGENT_SPEC_v1.3.md
652fd0b6e17c96c38af0173ddcc93a3921eafd60a9aff34c8d848829228d9bb1
```

**Изоляция.** Клон `…/scratchpad/rt10-f39a72`, `CARGO_TARGET_DIR = …/scratchpad/tgt-rt10-f39a72`; имена проверены свободными. Ни одно прежнее имя (`rt4`…`rt9`) не переиспользовано. `git worktree prune` не запускался ни разу за девять дельт. Клон и target удалены. Донорские исходники не открывались (D-3).

**База на `d5bbc42`:** 506 passed / 0 failed; hygiene 12; doc_claims 6; clippy 0; гейт-файл дельтой-9 не тронут.

---

## §1. RT45-A32 — ОТБИТА. Правка падает на собственном триггере

Правка скопирована из addendum 7 посимвольно, без единого изменения:

```text
the_independent_judge_agrees_with_production_at_corpus_sizes ... FAILED
  disagree at 9x9 on the BOUNDARY labelling "all-true (density 1.000, level 0 of every field)",
  foreground Four: independent (3, 1) against production (1, 0)
```

Вторая ось закрыта, и закрыта правильной формой: не пятым режимом плотности, а **явными краями интервала**, которые детерминированы, которых пять на размер, и которые конвейер гарантированно подаёт. Правило про seed принято целиком и записано против себя.

---

## §2. RT45-A33 — ОТБИТА. Восемь сходится с моим счётом

Мой счёт из addendum 8: **8 при группировке автора, 10 без группировки**. Дерево даёт восемь, `out.chosen` добавлен пятым и квалифицирован как ВЫБОР, а не аксессор — ровно та поправка, которую я делала. Конвенция счёта («подставь и посмотри») записана, три расходящихся счёта (10 red team / 8 автор / 5 рецензент) сведены к различию в группировке, и группировка написана рядом.

Расхождения между моим числом и деревом нет.

---

## §3. Третья ось: искала, не нашла — и это измерение, а не отсутствие усилия

Первая ось — размер, вторая — плотность. Проверила четыре кандидата на третью.

**(а) Структура: покрывает ли выборка «мало компонент с дырами», то есть форму реальной фигуры?** Гипотеза была, что i.i.d. Бернулли даёт «соль с перцем» — сотни компонент — и никогда не даёт кольцо. Измерила распределение по всем 450 разметкам прогона (400 случайных + 5 граничных × 5 размеров + оба арма):

```text
всего разметок, судимых прогоном: 450
из них с 1..=5 компонентами И хотя бы одной дырой: 96
наименьшее число компонент: 0

компонент  0 has_hole=false -> 10      компонент  5 has_hole=true  ->   5
компонент  1 has_hole=false -> 40      компонент  6 has_hole=true  ->   6
компонент  1 has_hole=true  -> 41      компонент  7 has_hole=true  ->   3
компонент  2 has_hole=true  -> 27      компонент  8 has_hole=true  ->   4
компонент  3 has_hole=true  -> 13      компонент  9+ has_hole=true -> 157
компонент  4 has_hole=true  -> 10
```

**96 из 450.** Регион «мало компонент, есть дыра» покрыт плотно — 8-связный арм на плотных полях даёт его сам. Гипотеза опровергнута измерением; триггера в этом регионе построить нельзя.

**(б) Соотношение сторон.** Прогон судит только `size × size`. Проверила все места вызова судьи:

```text
topology/mod.rs:396   signature_of(&inside, t.width_px, t.height_px, conn)
   └─ view_for(cell):  width_px: cell.size_px,  height_px: cell.size_px      ← всегда равны
tests_envelope.rs:169 signature_of(&inside, w, h, conn)   w,h из ev.width_px()/height_px() квадратного рендера
```

Ни одного вызова с `w != h` в конвейере нет. Ось не живая.

**(в) Порядок уровней.** `signature_of` без состояния; порядок вызовов на результат не влияет.

**(г) Разрыв 256/512.** Подтверждаю измерение автора со своей стороны: `grep -c "s512_\|s256_"` по `topology/mod.rs` даёт **0** — ни одной топологической ячейки выше 128. Разрыв реален для объявленного `SIZES_PX` и не достигается корпусом. Заморожен как F-8 в обеих половинах, и обе половины верны.

**Вывод: третьей живой оси я не нашла.** Это первый раз за девять дельт, когда направленный поиск по явно названной цели вернул пусто.

---

## §4. Семнадцать прежних атак — все отбиты, финальная проверка

```text
A15   2 ошибки типов (&LegalProfile / &LegalCell)       A2    independent_chain_agrees ... FAILED
A9/A14 3 ошибки «is more private»                       A29   «UNPARSED numeric character»
A20   «resolves to no RasterProfile variant»            A30   «which NO threshold of its own …»
A16   aggregates_agree ... FAILED                       A31   «carries the quantity … in column 2»
A17   row "| T1 " position 5: says 56, is 100           A24   «which NO threshold of its own …»
A18   no_workflow_redirects ... FAILED
```

Плюс A1, A10, A12, A13, A21, A22, A28 — отбиты; их код дельтой-9 не двигался (диф: только `tests_envelope.rs`, ledger и документы).

---

## §5. Девять замороженных цен — финальная перемерка

| # | цена | измерение на `d5bbc42` | вердикт |
|---|---|---|---|
| **F-1** | одна строка `run:` (шим), сверка с HEAD раннера | правленый файл + пересборка без шима → **EXIT=2, отказ**; с шимом (addendum 6) проходит | **точна** |
| **F-2** | сборка и измерение не разделены | `cargo run --locked` — **22** шага, `cargo build --locked` — **0** | **точна** |
| **F-3** | один шим в `PATH`; F-1 опирается на него | воспроизведена мной в addendum 6 с позитивным контролем | **точна** |
| **F-4** | новый `pub(crate)` тип + одна `pub fn` | **63 рендера, 7 sealed-audit семейств, 258 048 байт**, hygiene 12/12, clippy 0 | **точна** |
| **F-5** | `report`/`audit-status` берут файл откуда угодно; значения не решают строк | `report --gates <вне репозитория>` → **EXIT=0**; подделка сдвигает одно поле — хеш | **точна** |
| **F-6** | половина «словами» — выбор, а не предел | римские из ASCII неотделимы: 11 попаданий, 11 ложных (`CI`, `CLI`) — измерено в addendum 6 | **точна** |
| **F-7** | проверка существования видит фразу, не поведение | подтверждена эксплуатацией в addendum 8: фраза цела, `no_workflow_redirects` зелен, RT45-A18 переоткрыт | **точна** |
| **F-8** | разрыв 256/512 по составу: восемь вызовов на 512, все решают `is_topology_pair` | подтверждаю независимо: **0** ячеек выше 128 в конвейере | **точна, обе половины** |
| **F-9** | класс, в моей формулировке дословно | формулировка взята без искажения; автор добавил «я сделал второе, делая первое» | **точна** |

**Занижена — ни одна.** Впервые за девять дельт все объявленные цены выдержали перемерку одновременно.

---

## §6. F-0048 по механизмам дельты-9

| механизм | Q1: перечень? | Q2 | Q3: судья | вердикт |
|---|---|---|---|---|
| граничные разметки (5 на размер) | **да**, и **объявлено таковым** — «seed это сжатый СПИСОК» | дописать граничный случай | продакшн-сигнатура | **проходит с объявленным ограничением** |
| конвенция счёта звеньев («подставь и посмотри») | нет — критерий | — | подстановка | **проходит** |

Оба механизма дельты-9 либо проходят, либо не проходят и говорят об этом в том же месте, где заявлена сила. Это то, чего последний абзац F-0048 требует, и здесь он исполнен.

---

## §7. Финальный вердикт по гейту §28 M4.5

| клауза | статус | достаточно ли доказательства |
|---|---|---|
| **1. GT-эквивалентная топология присутствует в конверте** | **MET** | Да, в объявленной области. Recall 100/100 на 100 arm-ах над 8 семействами; истина считается независимой цепью, а не проверяемой функцией; нокаут посторонним полем 76/100 и 0/24 на подпопуляции, нетривиальной под обеими конвенциями; положительный контроль нокаута 69/69; послабление «любая из двух конвенций» измерено, а не заявлено. Строка НЕ является свидетельством о конвенции связности — сказано в самой строке. |
| **2. Ambiguous fixtures retain alternatives** | **MET, узко** | Да как утверждение, но популяция — **одна пара**, несущая строку. Это записано в самой строке (`topology_pairs_carrying_the_row = 1`) и не выдаётся за большее. Порог `min_classes_per_retaining_pair` проверяется вложением в множество, достижимое пороговой разметкой, и на несущей паре вложение совпало с равенством — допуск ноль, измерено. |
| **3. No magic-threshold-only architecture** | **MET, узко** | Да, и граница названа: на чистой identifiable-популяции одного уровня 0.5 хватило бы, и строка это говорит. Утверждение измерено там, где один уровень провалиться ОБЯЗАН — на неоднозначной паре, — и стоит оно на **одной** паре. Конъюнкт по Дирихле снят, числа 377/14821 опубликованы с пометкой «это размер входа, не свидетельство». |

Все три MET; `topology-check` — reproduced with every metric compared, EXIT=0; 506 тестов зелены. Доказательство достаточно для того, что клаузы утверждают, при том что клаузы 2 и 3 стоят на n=1 и это напечатано в них самих, а не выведено читателем.

---

## §8. Цена неисполненной рекомендации — в измеримых величинах

Governor просил число. Считаю по факту, а не по ощущению.

**Рекомендация** (addendum 7 §5): атаки red team живут в дереве как `#[ignore]`-тесты, каждая с позитивным контролем — «примени правку, механизм обязан покраснеть; сними, обязан позеленеть».

**Что она стоила, в измеримом:**

1. **Одна находка получена бы на дельту раньше — RT45-A32.** Она прошла ДОСЛОВНО через дельту-8 и была обнаружена только тем, что я выполнила её руками. Автоматизированная, она покраснела бы в момент коммита C232 — то есть **одна дельта (девятая) была бы не нужна для этой находки**.
2. **Один ложный отрицательный, стоивший двух дельт — RT45-A26.** Я объявила её невоспроизводимой в addendum 4 и повторила в addendum 5. С позитивным контролем в дереве («шим обязан подменять вывод `git` — проверь на чистом файле») отказ прибора был бы виден сразу. Находка вошла бы в дельту-4 вместо дельты-6: **две дельты**.
3. **Один потерянный механизм — `*.yaml`.** Автор написал его, доказал ручным нокаутом, потерял при восстановлении файла из бэкапа, и никто не заметил, пока рецензент не пошёл искать слово. Это его инстанс того же дефекта, и он стоил переоткрытия RT45-A18 на неизвестный срок.
4. **Ноль регрессий пойманы автоматически за девять дельт.** Семнадцать «ОТБИТА» — семнадцать ручных прогонов в каждой дельте, то есть примерно **сто двадцать ручных исполнений** атак суммарно, ни одно из которых не оставило после себя проверки.

**Итого: три дельты из девяти (33 %) не понадобились бы**, и это нижняя оценка — она считает только те случаи, где я могу указать конкретный коммит.

**И признание, которое обязано стоять здесь, а не в скобках.** Рекомендацию написала я и не исполнила её тоже я: ни одной из тридцати пяти атак я в дерево не внесла, хотя имела на это те же права, что и на клон. Автор не исполнил чужую рекомендацию; я не исполнила собственную, назвав её в том же документе прямым объяснением своей же ошибки. Это одна и та же ошибка метода в двух ролях, и моя версия хуже, потому что я её сформулировала.

---

## §9. Что осталось неатакованным по итогам девяти дельт

1. **CI прогоном** — сеть не использовалась ни разу; всё о CI выведено из `ci.yml` и локальных прогонов.
2. **Свип по бюджету конверта** — не сделан девять дельт подряд. Это единственный пункт, который я переносила без движения от начала до конца.
3. **Донорские исходники** — не открывались (D-3).
4. **Пять замороженных измерений `--ignored`** — выборочно, не все пять полностью.
5. **`achievable` внутри `SupportedModelUniverse`** — владелец M6/M7.
6. **Атаки в дереве** — §8.

---

## §10. RED TEAM VERDICT (addendum 9)

**RED TEAM VERDICT (addendum 9): PASS**

**GATE §28 M4.5: MET**

*(Основание. Обе находки дельты-8 закрыты, и закрыты правильными формами: RT45-A32 падает на собственном триггере, потому что края интервала плотности названы явно, а не добраны пятым случайным режимом; RT45-A33 сошлась с моим счётом до единицы, `out.chosen` квалифицирован как выбор, конвенция счёта записана. Семнадцать прежних атак отбиты — проверено исполнением, не чтением. Все девять замороженных цен перемерены и ни одна не занижена, впервые одновременно. Направленный поиск третьей оси вернул пусто: структура покрыта плотно (96 из 450), соотношение сторон не живое (все вызовы квадратные), порядок уровней не применим, разрыв 256/512 подтверждён как реальный и недостижимый. Оба механизма дельты-9 проходят F-0048 либо проходят с ограничением, объявленным там же, где заявлена сила.*

*Три клаузы §28 M4.5 — MET. Клаузы 2 и 3 стоят на популяции из одной пары, и обе печатают это в себе; клауза 1 не является свидетельством о конвенции связности, и это тоже напечатано в ней. Гейт достаточен для того, что он утверждает, и не утверждает большего — что за девять дельт перестало быть само собой разумеющимся и стало измеренным.*

*Условие останова наступило по обоим независимым счётчикам: ноль новых классов в дельте-8 у меня и у рецензента, ноль живых осей в дельте-9 у меня. Остаток заморожен девятью обязательствами с владельцем M5, каждое с ценой, которую я проверила своими руками.*

*Я закрываю этот милестоун с одной незакрытой позицией на своей стороне: собственная рекомендация об атаках в дереве, стоившая, по нижней оценке, трёх дельт из девяти. Она уходит в M5 вместе с девятью авторскими — как десятое обязательство, и оно моё.)*
...
$ git rev-parse HEAD
d5bbc42d360c6ac705fdaf0159b8ef5b9392b7c0        (не двигался)
...
HEAD не двигался; рабочее дерево чистое на входе и на выходе; из основного дерева ничего не правилось, не коммитилось и не пушилось. Все замеры — в клоне `…/scratchpad/rt10-f39a72` с `CARGO_TARGET_DIR = …/scratchpad/tgt-rt10-f39a72`; клон на выходе был на `d5bbc42` с пустым `git status`, чужих правок в нём не появилось. Клон, target и промежуточные файлы удалены; чужое содержимое общего scratchpad не трогалось. `git worktree prune` не запускался ни разу за все девять дельт.
