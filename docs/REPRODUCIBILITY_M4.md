...
# интерьера). С C154 они живут в ИНТЕГРАЦИОННОМ тесте — то есть в отдельном
# крейте, который физически не может назвать широкую популяцию корпуса
# (условие D1). Они помечены #[ignore] и не входят в путь по умолчанию:
