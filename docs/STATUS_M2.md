...
C046–C050 (дельта-3), C051 (артефакты re-gate 3),
**C052–C054 (дельта-4)**.
...
## 0c. Re-gate дельты-3 и дельта-4: граница, которая выводила класс из-под проверки

Re-gate дельты-3: **A = ACCEPT, B = ACCEPT, red team = FAIL** (артефакты —
C051). Оба cold review проверили больше, чем я заявлял: A измерил
ослабленное заявление об арбитре ТОЧНОЙ рациональной истиной (Python
Fraction) и сверх того проверил направление — хуже в каждой строке именно
арбитр, а не renderer; B подтвердил бит-нейтральность C046 независимо
(его seal-digests одинаковы на четырёх HEAD-ах подряд); red team
подтвердил её корпусным дайджестом на 20 000 входах и не нашёл третьего
экземпляра формы `(a−b)/(c−d)`.

**Блокер F-M2-R13 — и он о моей добросовестности, а не об арифметике.**
Предикат `is_resolvable`, введённый в C048, выводил из-под
differential-свойства ровно тот класс, где аккумулятор молча неверен.
Обоснование («три реализации дают 0 / +0.278 / −0.517 — значит общий
предел представления») оказалось неверным в двух точках из трёх: +0.278
верен, 0 был нашим дефектом, а −0.517 — артефакт МОЕГО суперсэмплера. Я
применил собственное мета-правило M-4 к двум инструментам и не применил к
третьему, своему же, а затем на его показании построил границу.

| # | Находка | Коммит | Существо |
|---|---|---|---|
| F-M2-R13 | **MAJOR** | C052 | `x_at` вычислял `1e150 + 1e300·(−1e−150)` — катастрофическая отмена; ответ 0 вместо 0.278 (28 % пикселя). Это обусловленность ВЫБРАННОЙ формулы, а не предел f64: интерполяция от БЛИЖНЕГО конца берёт величину точно. При `t ≤ 0.5` выражение тождественно прежнему, поэтому точность обычной геометрии не потеряна и **ни один замороженный digest не сдвинулся**. Закрывает D-5 и D-6 (оказались одним пунктом) |
| — | граница | C053 | Модель заменена **измерением**: сдвиг всех координат на 1 ulp и вопрос независимому эталону, насколько сдвинулся ЕГО ответ. Та же величина даёт границу согласия (замечание 4b). Проверено на самой находке: заявление red team «исключение на 100 % вызвано первым членом» **не подтверждаю** — оба члена равны 2.22e134 (в addendum-е описка 2.2e-166), поэтому простое сужение фикстур бы не вернуло; измеряемый критерий — возвращает |
| — | арбитраж | C053 | Новый критерий немедленно нашёл ошибку **в моём эталоне**: на вырожденном шипе `[A,B,B,A]` SH-эталон даёт −1 на пиксель с ~1e50, а аккумулятор точный 0 на всех магнитудах. Раз неправыми уже были ОБЕ стороны, расхождение теперь арбитрируется третьим методом; виновны мы только если расходимся и с ним. Проверено полным откатом F-0014: свойство падает само, арбитр указывает на нас |

**Golden digests в дельте-4: НЕ изменились** — ни один (0 изменённых
hex-строк; `t ≤ 0.5` — прежнее выражение бит-в-бит).
...
cargo test --release --workspace                         207 passed / 0 failed
```

Разбивка (173 в первой подаче → 195 → 201 → 206 → **207** после
дельты-4): vice-bench 20; vice-geom 43 (31 unit + 6 property predicates +
...
| G11 | Независимый REVIEW_M2 (clean checkout) ×2 разных model families + numerical/topology red-team pass (§34) | **ОТКРЫТ — блокирует M3** | Подача 1 (`db1a18a`): A **REJECT**, B ACCEPT-с-условиями, RT **FAIL**. Дельта-1 (`9f7d523`): A **ACCEPT**, B **ACCEPT**, RT **FAIL** (новый MAJOR — регрессия дельты-1). Дельта-2 (`bd859cd`): A **ACCEPT**, B **ACCEPT**, RT **FAIL** (пятый экземпляр класса + слепота новых property-тестов). Дельта-3 (`472a02c`): A **ACCEPT**, B **ACCEPT**, RT **FAIL** (граница выводила класс из-под проверки). Дельта-4 (C052–C054): **требуется финальный red-team re-check**; подписи A и B даны на дельту-3. Автор не самосертифицирует (§32 п.29) |
...
### Ограничение области дельты-4 (для решения, звать ли cold review повторно)

Дельта-4 не выходит за перечень находок re-gate 3. Затронуто:
`coverage.rs` (`interpolate_position` + вызов в `x_at`), тестовый эталон
(`tests/common`: третий метод-арбитр), `coverage_props.rs` (измеряемый
критерий, арбитраж, изъятое заявление), governance-документы. **Golden
digests не изменились ни на одну строку**, публичные сигнатуры не
менялись, арифметика для `t ≤ 0.5` бит-в-бит прежняя.
...
Дельта-3 получила ACCEPT обоих cold review и FAIL red team в третий раз, и
эта находка — не про арифметику: **введённая мной граница выводила из-под
проверки ровно тот класс, где аккумулятор молча неверен**, а её
обоснование опиралось на показание инструмента, который я не измерил
(своего же суперсэмплера). Заявление изъято, а не смягчено; правило M-4
теперь применено и к третьей стороне.
...
## Addendum (M3, C086) — G11 закрыт; форма записи исправлена

Этот addendum добавлен ПОСЛЕ подписей M2 и не меняет ни строки отчёта,
который подписывали ревьюеры.

**Что произошло.** В C057 (начало M3) я отредактировал строку G11 таблицы
§3 «ОТКРЫТ → ЗАКРЫТ». Содержание было верным, но форма — нет: артефакт, на
который ссылаются три подписи, перестал быть побайтово тем, что подписывали.
REVIEW_M3 M3-N11 указал, что чище было бы дописать addendum, как это сделано
в самих REVIEW_*/REDTEAM_*. Согласен: строка возвращена дословно, а факт
записан здесь.

**Факт.** Gate G11 (§34: два независимых cold review разных model families +
numerical/topology red-team pass) **ЗАКРЫТ** на HEAD `f939a0f` (дельта-4,
C052–C054) тремя подписями:

| Проверка | Вердикт | Документ |
|---|---|---|
| cold review A (Opus) | **ACCEPT** | `docs/REVIEW_M2_A.md`, addendum 4 |
| cold review B (Fable) | **ACCEPT** | `docs/REVIEW_M2_B.md`, addendum 4 |
| numerical/topology red team (Opus) | **PASS** | `docs/REDTEAM_M2.md`, addendum 4 |

Артефакты закоммичены дословно в C055; факт закрытия зафиксирован в C056.
Автор не самосертифицировал: все три подписи даны независимыми контекстами.

**Перенесено в M3:** M2-A-N8 / M2-B-N5 (долг D-4 — типизированный witness
сертификации вложения; закрыт в C062), M2-A-N10 (закрыт ещё в дельте-2), и
три пункта гигиены инструментария из REDTEAM_M2 addendum 4 (п. 2 и 3 закрыты
в M3; п. 1 — четвёртый метод — остаётся открытым, см. STATUS_M3 §5).

**Вынесенное правило.** Подписанный документ дополняется, а не правится.
Это относится и к строке, содержание которой бесспорно: подпись стоит под
байтами, а не под намерением.
