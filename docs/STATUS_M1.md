...
ACCEPT). Коммиты milestone: C006–C010.
...
Разбивка тестов: vice-geom 26 (20 unit + 6 property); vice-ir 66
(18 unit + 38 invalid-graph rejects + 7 property + 3 golden); vice-bench
19 (12 unit + 6 cli + 1 child-env). `#![forbid(unsafe_code)]` во всех
crates (плюс workspace-level forbid); модулей >800 LOC нет.
...
| G4 | **Invalid graphs rejected** (typed, §12) | PASS | `vice-ir/src/validate.rs`; 38 негативных тестов `validate_rejects.rs` (по одному сломанному инварианту, вкл. Euler/torus) |
...
| G11 | Независимый REVIEW_M1 подписан | **NOT DONE — блокирует M2** | ожидается `docs/REVIEW_M1.md` |
...
1. **Curve–curve intersection не сертифицируется.** Для пар сегментов с
   ...
   Самопересечение смежных кривых сегментов (общая вершина) — вне
   §12-инварианта, проверяется геометрической verification M2+.
...
5. **Tier A determinism only:** байтовая идентичность заявлена для одной
   платформы/бинаря; cross-platform Tier B — M12. Golden-тест на Linux CI
   ещё ни разу не исполнялся (см. blockers, B4 наследуется из M0).
...
1. **REVIEW_M1 отсутствует** — единственный блокер самого milestone:
   независимый clean-checkout review (§34), включая повторение property/
   golden-тестов и попытку сломать canonical determinism.
2. **B4 (наследуется из M0):** CI ни разу не исполнялся на GitHub (нет
   remote). Первый push обязан прогнать fmt/clippy/tests и M0-smoke на
   `ubuntu-latest`; для M1 это заодно первая кросс-платформенная проба
   golden digest (ожидание: PASS, т.к. формат зависит только от ryu/serde;
   расхождение = находка уровня blocker).
3. **B1/B2 (M0, дедлайн до M3):** v-ice build_failed baseline и
   несамодостаточный Vice- pin — reviewed-решения до M3; вместе с этой
   перезаписью выполняется N3-split hashes.json (нормативная/
   информационная секции) — см. REPRODUCIBILITY_M0.md.
4. Для M2 (renderer) потребуется решение о порте coverage-кода доноров:
   без license grant для v-ice порт запрещён (REVIEW_M0 усл. 6) —
   планировать clean-room либо внешние crates заранее.

## 6. Явное заявление об остановке

Автор НЕ самосертифицирует M1. Gate G11 открыт до независимого review
(spec §32 правило 29, §34). Никакой код M2 (renderer/coverage/tessellation
и т.д.) не начат: в workspace ровно три crates (vice-bench, vice-geom,
vice-ir).
