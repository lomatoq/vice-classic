...
**Новых ВНЕШНИХ зависимостей ноль.** Единственная запись, добавленная в
`Cargo.lock`, — новый крейт этого воркспейса:

| Крейт | Внешние зависимости | Где записаны |
|---|---|---|
| `vice-topology` | `serde`, `sha2`, `hex`, `thiserror` | §2 (M0) |

Все четыре уже используются в дереве, лицензии проверены по фактическим
распакованным пакетам в §2, и роль ни одной из них не изменилась
(`vice-topology` — библиотека, как и `vice-evidence`, которая тянет тот же
набор). Поэтому фактической проверки лицензий по пакетам M4.5 не требует, и
это записано именно как вывод из состава `Cargo.lock`, а не как умолчание.

PORTING_MANIFEST на M4.5 остаётся при **нуле units**. Ближайшие донорские
родственники того, что делает M4.5, — `vice_scene/topology.py` (`Vice-`) и
всё, что в донорских деревьях считает компоненты и дыры. **Ни один из этих
файлов не открывался**, и правило то же, что записал M4: смотреть на код
донора «чтобы свериться» — это и есть перенос, только неучтённый. Долг D-3
требует license/IP review ДО первого `[[unit]]`, а оба донора — под
`OWNER_CONTROLLED_VERIFY_BEFORE_PUBLIC_RELEASE`.

Отдельно, потому что это стоит сказать прямо: конвенция комплементарной
